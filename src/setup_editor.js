    ...
    vscode.commands.registerCommand(
      ...
      () => {
        ...
        if (vscode.workspace.workspaceFolders) {
          ...
          if (!fs.existsSync(dotvscodePath)) {
            ...
            vscode.window.showInformationMessage(
              `${dotvscodePath} has been created.`,
              ...['Ok']
            )
          } else {
            return vscode.window.showInformationMessage(
              `${dotvscodePath} already exists.`,
              ...['Ok']
            )
          }
        } else {
          return vscode.window.showErrorMessage(
            'Please open workspace folder first.',
            ...['Ok']
          )
        }
        ...
        EXT_TEMPLATE_FILES.forEach((filename) => {
          const extensionTemplateFilePath = path.join(
            extensionTemplatesPath,
            filename
          )
          ...
          if (!fs.existsSync(vscodeTemplateFilePath)) {
            fs.copyFileSync(
              extensionTemplateFilePath,
              vscodeTemplateFilePath,
              fs.constants.COPYFILE_EXCL
            )
            vscode.window.showInformationMessage(
              `${vscodeTemplateFilePath} created.`,
              ...['Ok']
            )
          } else {
            return vscode.window.showInformationMessage(
              `${vscodeTemplateFilePath} already exists.`,
              ...['Ok']
            )
          }
        })
